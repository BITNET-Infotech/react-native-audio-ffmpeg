...
exports.default = exports.AudioFFmpeg = void 0;
...
function getEmitter() {
  if (!_emitter) {
    ...
    _emitter = new _reactNative.NativeEventEmitter(Native());
  }
  return _emitter;
}
...
class AudioFFmpeg {
   ...
   */
  static async runAsync(command, callbacks) {
    const sessionId = newSessionId();
    const emitter = getEmitter();
    const subs = [];
    if (callbacks?.onLog) {
      subs.push(emitter.addListener('AudioFFmpegLog', e => {
        if (e.sessionId === sessionId) callbacks.onLog(e);
      }));
    }
    if (callbacks?.onProgress) {
      subs.push(emitter.addListener('AudioFFmpegProgress', e => {
        if (e.sessionId === sessionId) callbacks.onProgress(e);
      }));
    }
    if (callbacks?.onComplete) {
      subs.push(emitter.addListener('AudioFFmpegComplete', e => {
  ...
  static merge(inputs, output) {
    ...
    return Native().execute(`${inputArgs} -filter_complex "${filter}" -map [aout] ${codec} -y "${output}"`);
  }
   ...
   * });
   */
  static convert(input, output, options = '') {
    const optionFlags = typeof options === 'string' ? options : buildConvertFlags(options);
    const codec = optionFlags ? '' : codecFlagsFor(output);
    return Native().execute(`-i "${input}" -vn ${codec} ${optionFlags} -y "${output}"`);
  }
...
exports.AudioFFmpeg = AudioFFmpeg;
function buildConvertFlags(options = {}) {
  const flags = [];
...
var _default = exports.default = AudioFFmpeg;
//# sourceMappingURL=AudioFFmpeg.js.map